  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <span className="text-xs font-mono text-[#7a8db0] whitespace-nowrap">
        {pastCount}/{deadlines.length} in force
      </span>
    </div>
  );
}
          ...
          <div className="relative max-w-4xl mx-auto px-6 lg:px-12 text-center">
            <motion.div variants={fadeUp} initial="hidden" animate="show">
              ...
              <p className="text-lg text-[#94a3c4] max-w-xl mx-auto leading-relaxed mb-8">
                {DEADLINES.length} EU compliance deadlines at a glance.
                Filter by regulation, year or status — find exactly the deadlines that affect you.
              </p>
              ...
              {nextDeadline && (
                ...
                >
                  <span className="text-xs text-white/40">Next deadline:</span>
                  <span className="font-mono text-sm font-bold text-white">
                    {formatDateEN(nextDeadline.iso)}
                  </span>
            ...
            <div className="max-w-5xl mx-auto px-6 lg:px-12 py-4">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {(["all", "upcoming", "past"] as StatusFilter[]).map((s) => (
                  <FilterChip
                    key={s}
                    label={s === "all" ? "All" : s === "upcoming" ? "Upcoming" : "In force"}
          ...
          <div className="max-w-5xl mx-auto px-6 lg:px-12">
            {filtered.length === 0 ? (
              ...
              >
                ...
                <h3 className="font-[Syne] font-bold text-lg text-[#060c1a] mb-2">No deadlines found</h3>
                <p className="text-sm text-[#7a8db0] mb-4">Adjust the filters to see deadlines.</p>
                <button
                  onClick={resetFilters}
            ...
            <p className="text-center text-xs text-[#7a8db0] mt-10">
              As of: {new Date().toLocaleDateString("en-GB", { month: "long", year: "numeric" })}.
              Subject to change through delegated acts. All information without guarantee.
            </p>
          </div>
        </section>
        ...
        <section className="relative py-24 overflow-hidden">
          ...
          <div className="relative max-w-2xl mx-auto px-6 lg:px-12 text-center">
            <h2 className="font-[Syne] font-[800] text-3xl md:text-4xl text-white mb-4">
              Activate compliance briefing
            </h2>
            <p className="text-white/45 mb-8 max-w-md mx-auto">
              Your regulatory early warning system — only for critical deadlines and legislative changes. Maximum 3 times per month.
            </p>
      ...
      >
        ...
        {allPast ? (
          <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-mono font-bold bg-emerald-50 text-emerald-600 border border-emerald-200">
            ✓ {deadlines.length} in force
          </span>
        ) : (
          <span className="text-xs font-mono text-[#7a8db0]">
            {deadlines.filter((d) => !isPast(d.iso)).length} upcoming
          </span>
        )}
        <svg
