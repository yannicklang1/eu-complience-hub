...
interface RiskQuestion {
  ...
  inverse?: boolean; // true = "Ja" reduces risk
}

const RISK_QUESTIONS: RiskQuestion[] = [
  {
    id: "documented_compliance",
    question: "Gibt es ein dokumentiertes Compliance-Management-System?",
    description: "Formalisierte Richtlinien, Prozesse und Verantwortlichkeiten für Cybersecurity und Regulierungskonformität.",
    weight: 3,
    inverse: true,
  },
  {
    id: "ciso_appointed",
    question: "Gibt es einen CISO oder IT-Sicherheitsbeauftragten?",
    description: "Eine dedizierte Person (intern oder extern), die für Informationssicherheit verantwortlich ist.",
    weight: 2,
    inverse: true,
  },
  {
    id: "gf_training",
    question: "Haben Sie als GF eine Cybersecurity-Schulung absolviert?",
    description: "NIS2 Art. 20 und AI Act Art. 4 verlangen explizit Schulungen der Geschäftsleitung.",
    weight: 3,
    inverse: true,
  },
  {
    id: "risk_assessment",
    question: "Wurde eine formelle Risikoanalyse durchgeführt?",
    description: "Systematische Bewertung von IT-Risiken, dokumentiert und regelmäßig aktualisiert.",
    weight: 2,
    inverse: true,
  },
  {
    id: "incident_plan",
    question: "Existiert ein Incident-Response-Plan?",
    description: "Dokumentierter Notfallplan für Cybersecurity-Vorfälle mit Meldeketten und Verantwortlichkeiten.",
    weight: 2,
    inverse: true,
  },
  {
    id: "dando",
    question: "Haben Sie eine D&O-Versicherung?",
    description: "Directors & Officers Versicherung mit Cyber-Deckung und Compliance-Erweiterung.",
    weight: 1,
    inverse: true,
  },
  {
    id: "no_budget",
    question: "Fehlt ein dediziertes Budget für IT-Sicherheit?",
    description: "Kein festes Budget oder weniger als 5% des IT-Budgets für Security.",
    weight: 2,
    inverse: false,
  },
  {
    id: "supplier_mgmt",
    question: "Werden IT-Dienstleister und Lieferanten regelmäßig überprüft?",
    description: "Vertragliche Sicherheitsanforderungen, Audits und Bewertung der Lieferkettensicherheit.",
    weight: 2,
    inverse: true,
  },
];
...
function calculateRisk(
  ...
  const perRegulation = selectedRegulations.map((regId) => {
    ...
    const regRiskLevel: RiskLevel =
      score >= 60 ? "hoch" : score >= 30 ? "mittel" : "niedrig";

    let riskNote = "";
    if (regRiskLevel === "hoch") {
      riskNote = `Hohes Haftungsrisiko: ${reg.liabilityBasis}. Bußgeld bis ${reg.maxFine}.`;
    } else if (regRiskLevel === "mittel") {
      riskNote = `Mittleres Risiko: Grundlegende Maßnahmen vorhanden, aber Lücken bei ${reg.name}-spezifischen Anforderungen.`;
    } else {
      riskNote = `Geringes Restrisiko: Gute Grundlage vorhanden. Regelmäßige Überprüfung empfohlen.`;
    }
  ...
  if (!answers.documented_compliance) {
    recommendations.push("Etablieren Sie ein dokumentiertes Compliance-Management-System mit klaren Verantwortlichkeiten.");
  }
  if (!answers.gf_training) {
    recommendations.push("Absolvieren Sie eine Cybersecurity-Schulung — NIS2 Art. 20 und AI Act Art. 4 verlangen dies explizit von der Geschäftsleitung.");
  }
  if (!answers.ciso_appointed) {
    recommendations.push("Bestellen Sie einen CISO oder externen IT-Sicherheitsbeauftragten.");
  }
  if (!answers.risk_assessment) {
    recommendations.push("Führen Sie eine formelle IT-Risikoanalyse durch und dokumentieren Sie die Ergebnisse.");
  }
  if (!answers.incident_plan) {
    recommendations.push("Erstellen Sie einen Incident-Response-Plan mit 24h/72h Meldeketten (NIS2-Anforderung).");
  }
  if (!answers.dando) {
    recommendations.push("Prüfen Sie den Abschluss einer D&O-Versicherung mit Cyber-Compliance-Deckung.");
  }
  if (answers.no_budget) {
    recommendations.push("Definieren Sie ein dediziertes IT-Security-Budget (Empfehlung: mind. 10-15% des IT-Budgets).");
  }
  if (!answers.supplier_mgmt) {
    recommendations.push("Implementieren Sie ein Lieferanten-Risikomanagement mit vertraglichen Security-Anforderungen.");
  }

  if (recommendations.length === 0) {
    recommendations.push("Ihre Grundlage ist gut. Führen Sie regelmäßige Reviews durch und halten Sie Schulungen aktuell.");
  }

  return {
    level,
    score,
    color: level === "hoch" ? "#dc2626" : level === "mittel" ? "#d97706" : "#059669",
    bgColor: level === "hoch" ? "#fef2f2" : level === "mittel" ? "#fffbeb" : "#ecfdf5",
    borderColor: level === "hoch" ? "#fecaca" : level === "mittel" ? "#fde68a" : "#a7f3d0",
    title: level === "hoch" ? "Hohes Haftungsrisiko" : level === "mittel" ? "Mittleres Haftungsrisiko" : "Geringes Haftungsrisiko",
    description:
      level === "hoch"
        ? "Ihre aktuelle Aufstellung weist erhebliche Lücken auf. Als Geschäftsführer haften Sie persönlich und unbeschränkt."
        : level === "mittel"
        ? "Sie haben grundlegende Maßnahmen ergriffen, aber es bestehen noch relevante Lücken in Ihrer Compliance-Aufstellung."
        : "Sie haben eine solide Compliance-Grundlage geschaffen. Dennoch sollten Sie regelmäßig überprüfen und aktualisieren.",
    perRegulation,
    recommendations,
  };
}
              ...
              {!showResult ? (
                  ...
                  <div className="rounded-2xl border border-[#d8dff0] bg-white p-6 sm:p-8 shadow-sm">
                    ...
                    {step === 1 && (
                      <>
                        <StepHeader
                          number={2}
                          title="Bewerten Sie Ihre aktuelle Aufstellung"
                          description={`Beantworten Sie die folgenden ${RISK_QUESTIONS.length} Fragen ehrlich. ${answeredCount}/${RISK_QUESTIONS.length} beantwortet.`}
                        />
                    ...
                    {step === 2 && (
                      <>
                        <StepHeader
                          ...
                          description="Prüfen Sie Ihre Angaben und klicken Sie auf 'Risiko berechnen'."
                        />
                        <div className="space-y-3 mb-6">
                          ...
                          <div className="p-4 rounded-xl bg-[#f8f9fd] border border-[#e8ecf4]">
                            ...
                            <span className="font-[Syne] font-bold text-[14px] text-[#060c1a]">{answeredCount} von {RISK_QUESTIONS.length}</span>
                          </div>
                        </div>
                        <div className="p-4 rounded-xl bg-[#fff8e1] border border-[#f0e6c0]">
                          <div className="flex items-start gap-2">
                            ...
                            <p className="text-[12px] text-[#8a7020] leading-relaxed">
                              <strong>Disclaimer:</strong> Dieses Tool bietet eine erste Einschätzung. Es ersetzt keine professionelle Rechtsberatung.
                            </p>
                          </div>
                        </div>
                      </>
                    ...
                    <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#e8ecf4]">
                      ...
                      >
                        {step === TOTAL_STEPS - 1 ? "Risiko berechnen" : "Weiter"}
                ...
                >
                  ...
                  <div className="rounded-2xl border-2 p-6 sm:p-8 mb-6" style={{ background: assessment.bgColor, borderColor: assessment.borderColor }}>
                    ...
                    {/* Recommendations */}
                    <div>
                      <h3 className="font-[Syne] font-bold text-[14px] text-[#060c1a] mb-3">Handlungsempfehlungen</h3>
